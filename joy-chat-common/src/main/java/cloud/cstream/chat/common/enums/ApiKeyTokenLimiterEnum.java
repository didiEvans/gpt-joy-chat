...
public enum ApiKeyTokenLimiterEnum {
    ...
    GPT_3_5_TURBO_16K(ChatCompletion.Model.GPT_3_5_TURBO_16K, 16000),
    GPT_3_5_TURBO_16K_0613(ChatCompletion.Model.GPT_3_5_TURBO_16K, 16000),
    ...
    public static int getTokenLimitByOuterJarModelName(String outerJarModelName) {
        ApiKeyTokenLimiterEnum anEnum = load(outerJarModelName);
        if (Objects.nonNull(anEnum)){
            return anEnum.getTokenLimit();
        }
        return 0;
    }

    public static ApiKeyTokenLimiterEnum load(String modelName){
        return Arrays.stream(values()).filter(e -> e.getModel().getName().equals(modelName)).findAny().orElse(null);
    }
